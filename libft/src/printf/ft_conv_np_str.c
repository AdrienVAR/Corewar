...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2019/06/03 15:08:26 by cgiron            #+#    #+#             */
/*   Updated: 2019/06/03 15:08:28 by cgiron           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static void		ft_np_conv(t_master *mstr, int c, int tg)
{
	char base[9];
	...
	if (c <= 31)
	{
		...
		ft_update_buffer(mstr, base[c / 8]);
		ft_update_buffer(mstr, base[c % 8]);
	}
	else if (c == 127)
		ft_str_to_buffer(mstr, "\\177");
	else
	{
		tg == 1 ?
			ft_update_buffer(mstr, c) :
			ft_add_uni_buffer(mstr, c);
	}
}

void			ft_conv_np_str(t_master *mstr)
{
	wchar_t		*strl;
	char		*str;
	int			strlen;
